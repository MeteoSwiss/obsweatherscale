...
class TransformedHeteroskedasticNoise(TransformedNoise, HeteroskedasticNoise):
    ...
    def forward(
        ...
        **kwargs : Any
        ...
        """
        pure_noise_var = (
            super(HeteroskedasticNoise, self)
            .forward(*params, batch_shape=batch_shape, shape=shape, noise=noise)
    ...
    def forward(
        ...
        **kwargs : Any
        ...
        if shape is None:
            ...
            shape = (
                torch.Size(p.shape) if len(p.shape) == 1 else torch.Size(p.shape[:-1])
            )
        batch_shape, n = shape
